...
public final class JAXBUtil {
    ...
    private static final String schemaLocation = "/schema/mashup/visual-mashup.xsd";
    ...
    public static void initSchema() {
        Logger logger = LoggerFactory.getLogger(JAXBUtil.class);
        try {
            if (schema == null) {
                SchemaFactory sf = SchemaFactory
                        .newInstance(javax.xml.XMLConstants.W3C_XML_SCHEMA_NS_URI);
                schema = sf.newSchema(JAXBUtil.class.getResource(schemaLocation));

                if (logger.isDebugEnabled()) {
                    logger.debug("visual-schema.xsd Schema Initialized! : " + schemaLocation);
                }
            }
        } catch (Exception e) {
    ...
    /**
     * Obtains the {@link JAXBContext} from the given type, by using the cache if
     * possible.
     * <p/>
     * <p/>
     ...
     * Reads in a Java object tree from the given XML input.
     *
     * @param xml The URI is {@link URI#toURL() turned into URL} and then follows
     ...
     *            it's not {@link URI#isAbsolute() a valid absolute URI}, then it's
     ...
     *                   {@link Introspector#decapitalize(String) infered} from
     ...
     *                   {@link Class#getSimpleName() the short class name}. This
     *                   parameter must not be null.
     * @param xml        The XML will be {@link URLConnection#getOutputStream() sent} to
     ...
     *                   {@link HttpURLConnection HTTP URLs}, this will perform HTTP POST.
     * @throws DataBindingException If the operation fails, such as due to I/O error, unbindable
     *                              classes.
     */
     ...
     *                   {@link Class#getSimpleName() the short class name}. This
     *                   parameter must not be null.
     * @param xml        The XML will be sent to the given {@link OutputStream}. Upon a
     ...
     *                   {@link Writer}. Upon a successful completion, the stream will be
     ...
     *                   implementations. In case of {@link HttpURLConnection HTTP URLs}, this will
     *                   perform HTTP POST.</td>
     *                   </tr>
     *                   <tr>
     *                   <td>{@link URI}</td>
     *                   <td>The URI is {@link URI#toURL() turned into URL} and then follows the
     ...
     *                   <td>The XML will be sent to the given {@link OutputStream}. Upon a
