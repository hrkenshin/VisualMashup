 ...
 */
WireIt.Terminal = function(parentEl, options, container, extComponent, position) {
    // TODO : ADDED BY 이승백
    this.extComponent = extComponent;

    // TODO : ADDED BY 이승백(TOP | LEFT | BOTTOM | RIGHT)
...
WireIt.Terminal.prototype = {
   ...
   render: function() {
      ...
      // TODO : ADDED BY 이승백
      if(this.title) {
        this.el.innerHTML = '<div class="title">' + this.title.value + '</div>';
          if (lang.isObject(this.title.position)) {
              for (var key in this.title.position) {
                  if (this.title.position.hasOwnProperty(key) && this.title.position[key] !== "") {
                      this.el.firstChild.style[key] = this.title.position[key] + "px";
                  }
              }
          }
      }
	...
	move: function(position) {
      // Set the offset position
	  this.options.offsetPosition = position;
   ...
   getXY: function() {
      ...
      // TODO : MODIFIED BY 이승백
      var layerEl = this.container && this.container.layer ? this.container.layer.el : (this.extComponent ? Ext.getDom(this.extComponent.body):document.body);
